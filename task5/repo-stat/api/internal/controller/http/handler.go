...
func NewHandler(ctx context.Context, log *slog.Logger, cfg config.Config) (http.Handler, error) {
	...
	repoInfoUseCase := usecase.NewRepoInfo(processorClient)
	subscriberUseCase := usecase.NewSubscriptionManager(subscriberClient)

	mux := http.NewServeMux()
	AddRoutes(mux, log, pingUseCase, repoInfoUseCase, subscriberUseCase)
